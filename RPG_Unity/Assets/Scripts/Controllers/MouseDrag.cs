...
{
	public	RectTransform		dragRectangle;			// 마우스로 드래그한 범위를 가시화하는 Image UI의 RectTransform
	...
	{
		// 모든 유닛을 검사
		foreach ( UnitController unit in Managers.Unit.UnitList )
		{
			// 유닛의 월드 좌표를 화면 좌표로 변환해 드래그 범위 내에 있는지 검사
			if ( dragRect.Contains(Camera.main.WorldToScreenPoint(unit.transform.position)) )
			{
				Managers.Unit.DragSelectUnit(unit);
			}
		}
	}
}
